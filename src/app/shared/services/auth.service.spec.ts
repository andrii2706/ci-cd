...
describe('AuthService', () => {
  ...
  const mockAuth = {
    ...
    signInWithEmailAndPassword: jest.fn(() => of({ user: { email: 'test@example.com' } })),
    signInWithPopup: jest.fn(() => of({ user: { uid: 'testUid' } })),
    createUserWithEmailAndPassword: jest.fn(() => of({ user: { uid: 'testUid', email: 'test@example.com' } })),
    signOut: jest.fn(() => of({})),
    currentUser: {
    ...
    firestoreMock = {
      ...
      collection: jest.fn().mockReturnValue({
		...
		TestBed.configureTestingModule({
			imports: [AngularFireModule.initializeApp(environment.firebaseConfig), BrowserAnimationsModule, NoopAnimationsModule],
      providers: [
