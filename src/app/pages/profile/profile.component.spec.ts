...
describe('ProfileComponent', () => {
	...
	beforeEach(async () => {
		authServiceMock = {
			...
			getAvatarById: jest
				.fn()
				.mockResolvedValue({ photoUrl: 'mock-avatar-url' }),
	...
	it('should navigate to game details when goToGame is called', () => {
		...
		expect(routerMock.navigate).toHaveBeenCalledWith([`/games/${gameId}`], {
			relativeTo: activatedRouteMock,
		});
	});

	it('should update user info and handle errors', async () => {
		component.updateUserInfoForm.setValue({
		...
		expect(authServiceMock.updateUserInfo).toHaveBeenCalledWith(
			'Updated Name',
			'updated@example.com',
			'mock-avatar-url'
		);
