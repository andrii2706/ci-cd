...
describe('BotComponent', () => {
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      imports: [BotComponent, ReactiveFormsModule, BrowserAnimationsModule, NoopAnimationsModule], // Standalone компонент додається в imports
      providers: [
  ...
  it('should send a message and clear the input', () => {
    ...
    expect(component.messages).toEqual([{"message": "Hello, Bot!", "user": "user"}, {"message": "Hi there!", "user": "bot"}]);
  ...
  it('should handle bot response correctly', () => {
    ...
    expect(component.messages).toEqual([{"message": "Hello, User!", "user": "bot"}]);
  });

  it('should disconnect the bot after a delay', () => {
    jest.useFakeTimers(); // Використання фейкових таймерів
    botServiceMock.getMessages.mockReturnValue(of({ response: 'Disconnected!' }));
